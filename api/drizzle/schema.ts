...
export const order_votes = pgTable(
  "order_votes",
  {
    ...
    order_id: uuid("order_id")
      .notNull()
      .references(() => orders.id, { onUpdate: "cascade" }),
  ...
  (table) => {
    return {
      ...
      order_id_idx: index("order_votes_order_id_idx").on(table.order_id),
    };
  }
);
...
export const manager_withdraw_transactions = pgTable(
  "manager_withdraw_transactions",
  {
    ...
    transaction_id: uuid("transaction_id")
      .notNull()
      .references(() => order_transactions.id, { onUpdate: "cascade" }),
    amount: doublePrecision("amount").notNull(),
    created_at: timestamp("created_at", {
      precision: 5,
      withTimezone: true,
...
export const daily_garant = pgTable("daily_garant", {
  id: uuid("id").defaultRandom().primaryKey().notNull(),
  name: text("name").notNull(),
  date: time("date", { precision: 5, withTimezone: true }).notNull(),
  amount: doublePrecision("amount").notNull(),
...
export const order_actions = pgTable(
  ...
  (table) => {
    return {
      id_key: uniqueIndex("order_actions_id_key").on(table.id),
      PK_order_actions_id_terminal_id: primaryKey(table.id, table.terminal_id),
    };
  }
);
...
export const order_locations = pgTable(
  ...
  (table) => {
    return {
      ...
      PK_order_locations_id_terminal_id: primaryKey(
        table.id,
        table.terminal_id
      ),
    };
...
export const orders = pgTable(
  "orders",
  {
    ...
    order_status_id: uuid("order_status_id")
      .notNull()
      .references(() => order_status.id, { onUpdate: "cascade" }),
    ...
    wrong_lat: doublePrecision("wrong_lat").notNull(),
    wrong_lon: doublePrecision("wrong_lon").notNull(),
    ...
    cancel_voice_id: uuid("cancel_voice_id").references(() => assets.id, {
      onUpdate: "cascade",
    }),
  ...
  (table) => {
    return {
      ...
      PK_orders_id_terminal_id: primaryKey(table.id, table.terminal_id),
    };
  }
);
