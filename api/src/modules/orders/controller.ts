...
export const OrdersController = new Elysia({
  ...
  .get(
    "/api/orders",
    async ({
      query: { limit, offset, sort, filters, fields, ext_all },
      drizzle,
      user,
    }) => {
      ...
      if (filters) {
        ...
        whereClause.push(eq(terminals.active, true));
      }

      if (user?.terminals && user.terminals.length > 0) {
        whereClause.push(inArray(orders.terminal_id, user.terminals));
      }
      ...
      if (rolesList.length > 0) {
        ...
        const orderTransactions = await drizzle
          .select({
          ...
          .where(
            and(
              ...
              gte(
                ...
                dayjs(lastOrder.created_at).subtract(1, "day").toISOString(),
              ),
              lte(
                order_transactions.created_at,
                dayjs(firstOrder.created_at).add(5, "day").toISOString(),
              ),
            ),
          )
          .execute();
  ...
  .get(
    ...
    async ({ cacheControl, user, drizzle }) => {
      if (!user?.user.id) {
        return [];
      }
      ...
      const usersTerminals = await drizzle
        ...
        .execute();
      const terminalIds = usersTerminals.map(
        (terminal) => terminal.terminal_id,
      );
      const allTerminals = await cacheControl.getTerminals();
      const terminalsList = allTerminals.filter((terminal) =>
        terminalIds.includes(terminal.id),
      );
      const terminalsOrganizations = terminalsList.map(
        (terminal) => terminal.organization_id,
      );

      const orderStatuses = await cacheControl.getOrderStatuses();
      const organizationsOrderStatuses = orderStatuses.filter(
        (orderStatus) =>
          terminalsOrganizations.includes(orderStatus.organization_id) &&
          !orderStatus.finish &&
          !orderStatus.cancel,
      );
      const orderStatusIds = organizationsOrderStatuses.map(
      ...
      const ordersList = await drizzle
        ...
        .where(
          and(
            ...
            gte(
              orders.created_at,
              dayjs().subtract(4, "days").format("YYYY-MM-DD HH:mm:ss"),
            ),
            lte(
              orders.created_at,
              dayjs().add(2, "days").format("YYYY-MM-DD HH:mm:ss"),
            ),
          ),
        )
        .orderBy(asc(orders.created_at))
        .execute();
      let result = (await prepareOrdersNextButton(
        ordersList.map((order) => {
          if (order.orders_organization && order.orders_organization.icon_url) {
            order.orders_organization.icon_url =
              order.orders_organization.icon_url.replace(
                "model_uploads",
                "public/model_uploads",
              );
          }
          return order;
        }),
  ...
  .get(
    "/api/orders/my_new_orders",
    async ({ user, cacheControl, drizzle }) => {
      ...
      const usersTerminalsListPrepare = await drizzle
        ...
        .from(users_terminals)
        .where(eq(users_terminals.user_id, sql.placeholder("user_id")))
        .prepare("usersTerminalsList");
      const usersTerminalsList = await usersTerminalsListPrepare.execute({
        user_id: user?.user.id,
      });
      const orderStatuses = await cacheControl.getOrderStatuses();

      const newOrderStatuses = orderStatuses.filter(
        (orderStatus) => orderStatus.sort === 1,
      );

      const terminalsIds = usersTerminalsList.map(
        (terminal) => terminal.terminal_id,
      );
      const cacheTerminals = await cacheControl.getTerminals();
      const filteredTerminalIs = cacheTerminals.filter((terminal) =>
        terminalsIds.includes(terminal.id),
      );
      const terminalsOrganizations = filteredTerminalIs.map(
        (terminal) => terminal.organization_id,
      );

      const cacheOrganizations = await cacheControl.getOrganizations();

      const organizations = cacheOrganizations.filter((organization) =>
        terminalsOrganizations.includes(organization.id),
      );
      ...
      const userAdditionalData = await userMaxActiveOrderCountPrepare.execute({
        user_id: user?.user.id,
      });
      ...
      if (
        userAdditionalData.length > 0 &&
        userAdditionalData[0].max_active_order_count
      ) {
        maxActiveOrderCount = userAdditionalData[0].max_active_order_count!;
      } else {
        // get maximum max_active_order_count from organizations
        maxActiveOrderCount = Math.max(
          ...organizations.map(
            (organization) => organization.max_active_order_count,
          ),
        );
      }
      ...
      let possibleOrdersCount =
        maxActiveOrderCount - currentOrdersCount[0].count;
      ...
      const ordersList = await drizzle
        ...
        .where(
          and(
            ...
            inArray(
              orders.order_status_id,
              newOrderStatuses.map((orderStatus) => orderStatus.id),
            ),
            gte(orders.created_at, fromDate),
            lte(orders.created_at, toDate),
          ),
  ...
  .get(
    "/api/orders/list_in_map",
    async ({ drizzle, query, cacheControl, user }) => {
      ...
      const whereClause = [
        terminal_id
          ? inArray(orders.terminal_id, terminal_id.split(","))
          : undefined,
  ...
  .get(
    "/api/orders/filtered_list_in_map",
    async ({ drizzle, query, cacheControl, user }) => {
  ...
  .post(
    ...
    async ({
      ...
      drizzle,
      redis,
      queues: { processOrderChangeCourierQueue, processSetQueueLastCourier },
    }) => {
      if (!user?.user.id) {
        return [];
      }
      ...
      const distance = getDistance(
        {
          latitude: order.orders_terminals!.latitude,
          longitude: order.orders_terminals!.longitude,
        },
        { latitude, longitude },
      );
      ...
      const usersTerminalsList = await drizzle
        .select({
          terminal_id: users_terminals.terminal_id,
        })
        .from(users_terminals)
      ...
      const userAdditionalData = await drizzle
        .select({
          max_active_order_count: users.max_active_order_count,
        })
        .from(users)
      ...
      const currentStatusIndex = sortedOrderStatuses.findIndex(
        (orderStatus) => orderStatus.id === order.order_status_id,
      );
      ...
      await processOrderChangeCourierQueue.add(
        order.id,
        {
          order_id: order_id,
      ...
      await processSetQueueLastCourier.add(
        order.id,
        {
          courier_id: user?.user.id,
  ...
  .post(
    ...
    async ({
      body: { order_id, status_id, latitude, longitude },
      user,
      set,
      cacheControl,
      drizzle,
      ...
      const currentStatus = ordersStatuses.find(
        (orderStatus) => orderStatus.id === status_id,
      );

      const terminalsList = await cacheControl.getTerminals();
      const currentTerminal = terminalsList.find(
        (terminal) => terminal.id === order.terminal_id,
      );
      if (currentStatus?.in_terminal) {
        ...
        const distance = getDistance(
          {
            latitude: currentTerminal!.latitude,
            longitude: currentTerminal!.longitude,
          },
      ...
      if (currentStatus?.finish || currentStatus?.cancel) {
        const updateOrderStatusFinish = drizzle
          .update(orders)
          .set({
        ...
        const updateOrderPrepare = drizzle
          .update(orders)
          .set({
            order_status_id: status_id,
          })
      ...
      const result = (await prepareOrdersNextButton(
        ordersList.map((order) => {
          if (order.orders_organization && order.orders_organization.icon_url) {
            order.orders_organization.icon_url =
              order.orders_organization.icon_url.replace(
      ...
      await processOrderChangeStatusQueue.add(
        result[0].id,
        {
  ...
  .post(
    ...
    async ({
      ...
      try {
        ...
        const apiToken = apiTokens.find(
          (apiToken: any) => apiToken.token === token && apiToken.active,
        );
        if (!apiToken) {
          set.status = 403;
          ...
          const currentOrganization = organizations.find(
            (org: any) => org.id === apiToken.organization_id,
          );
          if (currentOrganization) {
            const searchOrder = await drizzle
              .select({
                id: orders.id,
              })
              .from(orders)
            ...
            const currentTerminal = terminals.find(
              (terminal: any) => terminal.external_id === terminal_id,
            );
            ...
            const deliveryPricing =
              await cacheControl.getOrganizationDeliveryPricing(
            ...
            const activeDeliveryPricing = deliveryPricing.filter((d) => {
              ...
              if (
                ...
                if (d.terminal_id === null) {
                  ...
                  if (
                    d.source_type &&
                    d.source_type.length > 0 &&
                    source_type &&
                    source_type.length > 0
                  ) {
                    const sourceTypes = d.source_type
                      .split(",")
                      .map((s) => s.trim());
            ...
            let activeDeliveryPricingSorted = sort(
              activeDeliveryPricing,
              (i) => +i.default,
            );
            activeDeliveryPricingSorted = sort(
              activeDeliveryPricingSorted,
              (i) => +i.price_per_km,
            );
            activeDeliveryPricingSorted = sort(
              activeDeliveryPricingSorted,
              (i) => (i.min_price ? +i.min_price! : 0),
              true,
            );

            const terminalDeliveryPricing = activeDeliveryPricing.filter(
              (d) => d.terminal_id === currentTerminal.id,
            );
            let terminalDeliveryPricingSorted = sort(
              terminalDeliveryPricing,
              (i) => +i.default,
            );
            terminalDeliveryPricingSorted = sort(
              terminalDeliveryPricingSorted,
              (i) => +i.price_per_km,
            );
            terminalDeliveryPricingSorted = sort(
              terminalDeliveryPricingSorted,
            ...
            const otherDeliveryPricing = activeDeliveryPricing.filter(
              (d) => d.terminal_id !== currentTerminal.id,
            );
            let otherDeliveryPricingSorted = sort(
              otherDeliveryPricing,
              (i) => +i.default,
            );
            otherDeliveryPricingSorted = sort(
              otherDeliveryPricingSorted,
              (i) => +i.price_per_km,
            );
            otherDeliveryPricingSorted = sort(
              otherDeliveryPricingSorted,
            ...
            let minDistance = 0;
            let minDuration = 0;
            let minDeliveryPricing: InferSelectModel<
              ...
              if (d.drive_type == "foot") {
                ...
                try {
                  ...
                  if (
                    ...
                    tempDistance > d.min_distance_km
                  ) {
                    minDistance = tempDistance;
                    minDuration = data.routes[0].duration;
                    minDeliveryPricing = d;
            ...
            const minDeliveryPricingRules = minDeliveryPricing?.rules as
              | DeliveryPricingRulesDto[]
              | undefined;
            ...
            const minDeliveryPricingCustomerRules =
              minDeliveryPricing?.customer_rules as
                | DeliveryPricingRulesDto[]
                | undefined;
            ...
            orderStatus = orderStatuses.find(
              (o) =>
                o.sort === 1 && o.organization_id === currentOrganization.id,
            );
            ...
            let courierAssignTime = await getSetting(
              redis,
              "courier_assign_time",
            );
  ...
  .post(
    ...
    async ({
      ...
      let query = (
        await drizzle.execute<GarantReportItem>(
      ...
      > = {};
      const couriersByTerminal = (
        await drizzle.execute<ReportCouriersByTerminal>(sql`select sum(o.delivery_price)                  as delivery_price,
                                    concat(u.first_name, ' ', u.last_name) as courier,
                                    o.courier_id,
                                    o.courier_id,
                                    o.organization_id,
      ...
      if (customDateCouriers.length) {
        ...
        const transaction = customDateCouriers.forEach(async (item) => {
          const customSqlStartDateTime = sql.raw(
            `${dayjs(item.order_start_date).format("YYYY-MM-DD")} 00:00:00`,
          );
          const customSqlEndDateTime = sql.raw(
            `${dayjs(sqlEndDate).format("YYYY-MM-DD")} 04:00:00`,
          );
      ...
      const courierSqlMap = sql.raw(
        courierIds.map((id) => `'${id}'`).join(","),
      );
      const balanceQuery = (
        await drizzle.execute<{
          courier_id: string;
      ...
      query.forEach((item) => {
        ...
        if (garantPrices) {
          const garantPriceObject = garantPrices.find(
            (priceItem) =>
              priceItem.drive_type === couriersById[item.courier_id].drive_type,
          );
        ...
        couriersById[item.courier_id].created_at = new Date(
          new Date(couriersById[item.courier_id].created_at).setHours(
            0,
            0,
            0,
            0,
        ...
        if (giveGarant) {
          ...
          const possibleGarantPrice =
            Math.round(((garantPrice / daysInMonth) * workDays) / 1000) * 1000;
  ...
  .post(
    ...
    async ({
      ...
      set,
      user,
      queues: { processTryAssignCourier },
    }) => {
  ...
  .post(
    ...
    async ({
      ...
      set,
      user,
      queues: { processOrderChangeCourierQueue },
    }) => {
      const order = await drizzle
        .select({
          id: orders.id,
  ...
  .post(
    "/api/orders/:id/revoke",
    async ({
      params: { id },
      drizzle,
      set,
      user,
      queues: { processClearCourierQueue },
    }) => {
      const order = await drizzle
        .select({
          id: orders.id,
      ...
      await processClearCourierQueue.add(
        order[0].id,
        {
          order_id: id,
      ...
      let order = await drizzle
        ...
        .where(
          and(
            ...
            lte(
              orders.created_at,
              dayjs(created_at).add(2, "hours").format("YYYY-MM-DD HH:mm:ss"),
            ),
          ),
        )
        .execute();
  ...
  .get(
    ...
    async ({
      ...
      const deliveryPricing = await cacheControl.getDeliveryPricingById(
        order[0].delivery_pricing_id as string,
      );
  ...
  .get(
    ...
    async ({ params: { id }, drizzle, set, user }) => {
      ...
      const customerCommentsPrepare = drizzle
        ...
        .where(
          eq(customers_comments.customer_id, sql.placeholder("customer_id")),
        )
  ...
  .post(
    "/api/orders/my_history",
    async ({
      drizzle,
      set,
      user,
      cacheControl,
      body: { startDate, endDate, limit, page },
    }) => {
      const courierIds: string[] = [];
      // @ts-ignore
      const courierRole = user.access.roles.find(
        (role) => role.code === "courier",
      );
      // @ts-ignore
      const managerRole = user.access.roles.find(
  ...
  .post(
    ...
    async ({ drizzle, set, user, cacheControl, body: { page, limit } }) => {
      // @ts-ignore
      const managerRole = user.access.roles.find(
        (role) => role.code === "manager",
      );
      ...
      for (const organization of organizations) {
        const onWayStatus = orderStatuses.find(
          (orderStatus) =>
            orderStatus.on_way &&
            orderStatus.organization_id === organization.id,
        );

        if (onWayStatus) {
          pendingStatusIds.push(
            ...orderStatuses
              .filter(
                (orderStatus) =>
                  orderStatus.organization_id === organization.id &&
                  orderStatus.sort < onWayStatus.sort,
              )
              .map((orderStatus) => orderStatus.id),
          );
        }
      ...
      const ordersCount = await drizzle
        ...
        .where(
          and(
            ...
            inArray(
              orders.terminal_id,
              usersTerminals.map((item) => item.terminal_id),
            ),
            gte(orders.created_at, startDate),
            lte(orders.created_at, endDate),
          ),
  ...
  .post(
    ...
    async ({
      ...
      set,
      user,
      queues: { processOrderChangeStatusQueue },
    }) => {
      const order = await drizzle
        .select({
          id: orders.id,
      ...
      const canceledOrderStatus = orderStatuses.find(
        (orderStatus) =>
          orderStatus.cancel && orderStatus.organization_id === organizationId,
      );
  ...
  .post(
    "/api/orders/:id/locations",
    async ({ params: { id }, body: { created_at }, drizzle, set, user }) => {
      const locations = (await drizzle
        ...
        .leftJoin(
          order_status,
          eq(order_locations.order_status_id, order_status.id),
        )
        .where(
          and(
            eq(order_locations.order_id, id),
            lte(
              order_locations.order_created_at,
              dayjs(created_at).add(2, "hours").format("YYYY-MM-DD HH:mm:ss"),
            ),
            gte(
              order_locations.order_created_at,
  ...
  .get(
    ...
    async ({ query, drizzle }) => {
      ...
      const formattedStats = terminalStats.rows.map((stat) => ({
        ...
        fastest_delivery: fancyTimeFormat(
          Number(stat.fastest_delivery_seconds),
        ),
        slowest_delivery: fancyTimeFormat(
          Number(stat.slowest_delivery_seconds),
        ),
        average_delivery: fancyTimeFormat(
          Number(stat.average_delivery_seconds),
        ),
