...
export const OrdersController = (
    app: Elysia<
        "",
        {
            store: {
                ...
                processOrderCompleteQueue: Queue,
            };
            bearer: string;
            request: {};
            schema: {};
    ...
    app
        ...
        .get(
            "/api/orders",
            async ({ query: { limit, offset, sort, filters, fields, ext_all } }) => {
        ...
        .get('/api/orders/my_orders', async ({ store: { redis, cacheControl }, user, set }) => {
            if (!user) {
                set.status = 400;
                return {
            ...
            const usersTerminals = await db.select({
        ...
        .get('/api/orders/my_new_orders', async ({ user, set, store: { redis, cacheControl } }) => {
            if (!user) {
                set.status = 400;
                return {
                    message: "User not found",
            ...
            const usersTerminalsList = await db.select({
                terminal_id: users_terminals.terminal_id
            }).from(users_terminals).where(eq(users_terminals.user_id, user.user.id)).execute();
            ...
            const userAdditionalData = await db.select({
            ...
            const currentOrdersCount = await db.select({
        ...
        .post('/api/orders/approve', async ({ body: { order_id, latitude, longitude }, user, set, store: { cacheControl, processOrderIndexQueue } }) => {
            if (!user) {
                set.status = 400;
                return {
                    message: "User not found",
        ...
        .post('/api/orders/set_status', async ({ body: { order_id, status_id, latitude, longitude }, user, set, store: { cacheControl, processOrderCompleteQueue, processOrderEcommerceWebhookQueue } }) => {
            if (!user) {
                set.status = 400;
                return {
                    message: "User not found",
            ...
            const lastOrderActions = await db
                .select({
                    id: order_actions.id,
