...
export default async function processYandexCallback(redis: Redis, db: DB, cacheControl: CacheControlService, data: any) {
    ...
    const order = await db.query.orders.findFirst({
        where: and(
            eq(orders.yandex_id, data.claim_id),
    ...
    if (order) {
        ...
        const orderStatusId = orderStatusByOrganization[order.organization_id][data.status];
        ...
        try {
            const yandexFetch = await fetch(`https://b2b.taxi.yandex.net/b2b/cargo/integration/v2/claims/info?claim_id=${data.claim_id}`, {
                method: 'POST',
                headers: {
                    'Accept-Language': 'ru',
                        ...
                        if (webhookUrl) {
                            ...
                            const webhookData: any = {
                                log: {
                                    ...
                                    courier_name: data?.performer_info?.courier_name ?? '',
                            ...
                            const webhookResponse = await webhookFetch.json();
                        }
                    }
                } catch (e) { }
