...
import { IStyle, IExcelColumn, ITbodyConfig, IDataSource } from "../app";
...
export function renderTbody(
  ...
  dataSource.forEach((data, index) => {
    ...
    for (let i = 0; i < allColumns.length; i++) {
      ...
      let cell: Cell = excel.sheet!.cell(
        excel.currentRow - 1,
        excel.currentCol + i
      );
      let hMerge = 0;
      ...
      if (renderValue !== null) {
        const { children, colSpan, rowSpan, __style__ } =
          getColumnRenderValue(renderValue);
        value = children;
        hMerge = colSpan;
        vMerge = rowSpan;
        cellStyle = __style__;
