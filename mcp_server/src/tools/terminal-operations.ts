...
import { terminals, organization, orders, users } from '../lib/schema.js';
...
export const terminalOperationsTool = {
      ...
      content: [
        {
          ...
          text: JSON.stringify({
            success: false,
            error: {
              message: `Failed to fetch terminals: ${error instanceof Error ? error.message : String(error)}`,
              code: 'DATABASE_ERROR'
            }
          }, null, 2)
        }
      ...
      content: [
        {
          ...
          text: JSON.stringify({
            success: false,
            error: {
              message: `Failed to fetch terminal details: ${error instanceof Error ? error.message : String(error)}`,
              code: 'DATABASE_ERROR'
            }
          }, null, 2)
        }
      ...
      content: [
        {
          ...
          text: JSON.stringify({
            success: false,
            error: {
              message: `Failed to update terminal status: ${error instanceof Error ? error.message : String(error)}`,
              code: 'DATABASE_ERROR'
            }
          }, null, 2)
        }
